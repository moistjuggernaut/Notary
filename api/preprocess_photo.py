...
import os
from api.face_check import Config, REMBG_AVAILABLE, remove_background_rembg
...
class ImagePreprocessor:
    ...
    def _calculate_crop_coordinates(self, img_shape, face_details):
        """Calculates precise crop coordinates on the original image."""
        ...
        if scale <= 0: return None, "Invalid crop scale factor."
    ...
    def _preliminary_background_check(self, image_bgr, face_bbox):
        ...
        if bg_pixels.size < 1000: return False, "Not enough background pixels to check." 
        ...
        reason = []
        if not is_light: reason.append("not light enough")
        if not is_uniform: reason.append("not uniform")
    ...
    def process_image(self, original_image_bgr):
        ...
        final_bgr = cv2.resize(cropped_bgr, (self.config.FINAL_OUTPUT_WIDTH_PX, self.config.FINAL_OUTPUT_HEIGHT_PX), interpolation=cv2.INTER_AREA)
