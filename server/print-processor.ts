...
import { ICAOConfig } from './validation-constants.js'
...
const PHOTO_WIDTH_PX = ICAOConfig.finalOutputWidthPx
const PHOTO_HEIGHT_PX = ICAOConfig.finalOutputHeightPx
...
function calculateLayout() {
  const totalPhotosWidth = PHOTO_WIDTH_PX * GRID_COLS + SPACING_PX * (GRID_COLS - 1)
  const totalPhotosHeight = PHOTO_HEIGHT_PX * GRID_ROWS + SPACING_PX * (GRID_ROWS - 1)
 ...
 */
function createOverlaySvg(marginX: number, marginY: number): string {
  ...
  for (let row = 0; row < GRID_ROWS; row++) {
    for (let col = 0; col < GRID_COLS; col++) {
      const x0 = marginX + col * (PHOTO_WIDTH_PX + SPACING_PX)
      const y0 = marginY + row * (PHOTO_HEIGHT_PX + SPACING_PX)
      const x1 = x0 + PHOTO_WIDTH_PX
      const y1 = y0 + PHOTO_HEIGHT_PX
  ...
  const infoText = '4x Passport Photos (35x45mm) - Cut along guides'
 ...
 *
 * @param photoBuffer - The processed passport photo as a PNG buffer
 * @returns PrintLayoutResult with the print-ready image
 */
export async function createPrintLayout(photoBuffer: Buffer): Promise<PrintLayoutResult> {
  try {
    const { marginX, marginY } = calculateLayout()
    ...
    for (let row = 0; row < GRID_ROWS; row++) {
      for (let col = 0; col < GRID_COLS; col++) {
        const x = marginX + col * (PHOTO_WIDTH_PX + SPACING_PX)
        const y = marginY + row * (PHOTO_HEIGHT_PX + SPACING_PX)
    ...
    const overlaySvg = createOverlaySvg(marginX, marginY)
